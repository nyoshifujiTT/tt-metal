...
// B1: the same single-core matmul as matmul_single_core/kernels/compute/mm.cpp, written out as
// the unpacker and math work it performs instead of calling the LLK matmul library.
//
// Neither llk_unpack_AB_matmul* nor llk_math_matmul* is called here. The unpack side programs the
// SrcA/SrcB datum counts and issues UNPACR itself; the math side programs the ADDR_MODs and issues
// the 16-MVMUL full-tile sequence itself, per fidelity phase, with no MOP and no replay buffer.
//
// Why: the planned zero-injection work has to change what lands in SrcA/SrcB between the unpack
// and the MVMUL sequence, and has to control the MVMUL issue order. Neither is reachable through
// matmul_tiles() or through the LLK matmul entry points, which hide both behind a MOP.
//
// This kernel must stay bit-exact with mm.cpp. The host checks that on three K-layouts, and the
// check itself was validated with a negative control (see README).
//
...
inline void custom_matmul_configure_addrmod() {
    ...
    // Step within a face: next 8 SrcB rows, next 8 DST rows, same SrcA face.
    ckernel::addr_mod_t{
    ...
    // Advance SrcA to the next face, rewind SrcB to its row marker.
    ckernel::addr_mod_t{
    ...
    // Rewind SrcA, advance SrcB by a full face.
    ckernel::addr_mod_t{
    ...
    // Cross from the top face row to the bottom one; DST returns to its row marker.
    ckernel::addr_mod_t{
...
// The full-tile MVMUL sequence: 16 MVMULs covering the four SrcB faces against the four SrcA
// faces, repeated once per fidelity phase.
inline void custom_matmul_one_tile() {
    ...
    for (uint32_t phase = 0; phase < phases; ++phase) {
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_0, 0);  // B0A0
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_1, 0);  // B0A0, SrcA -> A1
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_0, 0);  // B0A1
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_2, 0);  // B0A1, SrcB -> B2

        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_0, 0);  // B2A0
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_1, 0);  // B2A0, SrcA -> A1
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_0, 0);  // B2A1
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_4, 0);  // B2A1, cross to bottom

        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_0, 0);  // B1A2
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_1, 0);  // B1A2, SrcA -> A3
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_0, 0);  // B1A3
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_2, 0);  // B1A3, SrcB -> B3

        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_0, 0);  // B3A2
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_1, 0);  // B3A2, SrcA -> A3
        TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_0, 0);  // B3A3

        if constexpr (high_fidelity) {
            // Roll over to the next fidelity phase without releasing the operands.
            TTI_MVMUL(ckernel::p_setrwc::CLR_NONE, 0, ckernel::ADDR_MOD_5, 0);
        } else {
            // LoFi has a single phase, so the last MVMUL also releases SrcA.
            TTI_MVMUL(ckernel::p_setrwc::CLR_A, 0, ckernel::ADDR_MOD_5, 0);
        }
    }

    if constexpr (high_fidelity) {
        // All phases consumed: release SrcA. This is the MOP end-op in the LLK version.
        TTI_SETRWC(ckernel::p_setrwc::CLR_A, 0, 0, 0, 0, ckernel::p_setrwc::SET_ABD_F);
    }

    // End of the reuse row. With ct_dim == rt_dim == 1 there is exactly one iteration, so this
    // fires every call. SrcA is released above, SrcB here; both must be released or the unpacker
    // blocks waiting for the source registers to free up.
